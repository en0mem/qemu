...
int main(int argc, char **argv) {
    int scan_depth = -1;
    if (argc < 4) {
        printf("Usage: %s, <ns_dev> <level> <repetitions>\n", argv[0]);
        return 1;
    }
    ...
    int sector_size = 512;
    void *read_data = NULL;
    if (posix_memalign(&read_data, NVME_SECTOR_SIZE, sector_size) != 0) {
    ...
    for (int i = 0; i < repetitions; ++i)  {
        ...
        for (int i = 0; i < scan_depth; i++) {  
            ...
            uint64_t offset = (get_random_number() % NUM_BLOCKS);
            io_cmd.cdw10 = (uint32_t)(offset & 0xffffffff);         // LBA lower 32 bits
            io_cmd.cdw11 = 0;
